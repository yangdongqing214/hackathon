...
In the main hackathon app, this module is served on the logged-in **My allocation** dashboard (`http://localhost:5175/dashboard`) after `frontend` + `backend` are running. Discover then reads published nonprofits from the Express/Mongo API.

To run this folder on its own (SQLite demo data, no React shell):
